...
# No Password sudo confised -i 's/^%sudo.*/%sudo ALL=(ALL) NOPASSWD:ALL/g' /etc/sudoers
...
cp /etc/apt/sources.list /etc/apt/sources.list.d/realman_ros2.list
# Clear original software sources
echo "" > /etc/apt/sources.list
echo "" > /etc/apt/sources.list.d/realman_ros2.list
...
# Install Curl
apt-get install curl -y # If you haven't already installed curl
...
apt-get install pip -y # If you haven't already installed pip

# Install gnome-terminal
apt-get install gnome-terminal -y # If you haven't already installed gnome-terminal
...
if [ $Ubuntu_version = "20.04" ]; then
  ...
  apt-get install gazebo11 -y
  apt-get install ros-foxy-gazebo-* -y
elif [ $Ubuntu_version = "22.04" ]; then
